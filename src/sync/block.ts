...
export class WatchBlock implements WatchCat {
	...
	*/
	...
	private async solveBlock(blockNumber: number) {
		...
		if (chain == ChainType.MATIC && await web3.hasSupportGetTransactionReceiptsByBlock()) {
		...
		let logs = await web3.eth.getPastLogs({ toBlock: blockNumber, fromBlock: blockNumber });
