...
async function load_main_db() {
	...
	for ( let [k] of Object.entries(cfg.web3s)) {
		...
		await main_db.load(`
			...
			create table if not exists asset_global_${chain} (
				id           int primary key auto_increment,
				host         varchar (64)               not null, -- dao host
				token        varchar (64)               not null, -- address
			...
			create table if not exisis ledger_release_log_${chain} ( -- 成员分成日志
				id           int primary key auto_increment,
				address      varchar (64)                 not null, -- 合约地址
				operator     varchar (64)                 not null,
				txHash       varchar (72)                 not null, -- tx hash
			...
			create table if not exisis vote_proposal_${chain} ( -- 投票提案
				id           int primary key auto_increment,
				host         varchar (64)                 not null, -- dao host
			...
			create table if not exisis votes_${chain} ( -- 投票
				id           int primary key auto_increment,
				proposal_id  varchar (72)                 not null, -- 提案id
		...
		`, [], [
			// dao
			`create  unique index dao_${chain}_idx0              on dao                    (address)`,
			// member
			`create         index member_${chain}_idx1           on member                 (token)`,
			`create unique  index member_${chain}_idx2           on member                 (token,tokenId)`,
			`create         index member_${chain}_idx3           on member                 (token,owner)`,
			`create         index member_${chain}_idx4           on member                 (owner)`,
			`create         index member_${chain}_idx5           on member                 (name)`,
			// asset globl
			`create         index asset_global_${chain}_idx0     on asset_global           (token)`,
			`create  unique index asset_global_${chain}_idx1     on asset_global           (token,tokenId)`,
			`create         index asset_global_${chain}_idx2     on asset_global           (token,owner)`,
			// ordr
			`create         index asset_order_${chain}_idx0      on asset_order            (token)`,
			`create         index asset_order_${chain}_idx1      on asset_order            (token,tokenId)`,
			`create         index asset_order_${chain}_idx2      on asset_order            (token,fromAddres)`,
			`create         index asset_order_${chain}_idx3      on asset_order            (token,toAddres)`,
			// ledgr
			`create         index ledger_${chain}_idx0           on ledger                 (address)`,
			// `create         index ledger_${chain}_idx1           on ledger                 (txHash)`,
			`create         index ledger_${chain}_idx1           on ledger                 (address,target)`,
			`create         index ledger_${chain}_idx2           on ledger                 (address,type)`,
			`create         index ledger_${chain}_idx3           on ledger                 (address,target,type)`,
			`create         index ledger_${chain}_idx4           on ledger                 (address,txHash,type,member_id)`,
			// ledger_release_log
			`create unique  index ledger_release_log_${chain}_idx0  on ledger_release_log_${chain} (address,txHash)`,
			// vote_proposl
			`create         index vote_proposal_${chain}_idx0    on vote_proposal          (address)`,
			`create unique  index vote_proposal_${chain}_idx1    on vote_proposal          (address,proposal_id)`,
			`create         index vote_proposal_${chain}_idx2    on vote_proposal          (address,origin)`,
			// vots
			`create         index votes_${chain}_idx0            on votes                  (address,proposal_id)`,
			`create unique  index votes_${chain}_idx1            on votes                  (address,proposal_id,member_id)`,
			`create         index votes_${chain}_idx2            on votes                  (address,member_id)`,
		], `shs_${chain}`);
	}
}
