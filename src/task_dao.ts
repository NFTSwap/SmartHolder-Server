...
import {Task,TaskData} from 'bclib/task';
...
import {select, insert as insertC, update as updateC} from './models/contract';
// abis
import * as ContextProxyDAO from '../abi/DAOProxy.json';
import * as ContextProxyAsset from '../abi/AssetProxy.json';
import * as ContextProxyAssetShell from '../abi/AssetShellProxy.json';
import * as ContextProxyLedger from '../abi/LedgerProxy.json';
import * as ContextProxyMember from '../abi/MemberProxy.json';
import * as ContextProxyVotePool from '../abi/VotePoolProxy.json';
...
export class MakeDAO extends Task<MakeDaoArgs> {
	...
	constructor(tasks: TaskData<MakeDaoArgs>) {
		super(tasks);
	}
	...
	exec(args: MakeDaoArgs) {
		...
		this.step(async ()=>{
			...
			if (! await select(address, web3.chain)) {
...
host=${DAO}&chain=${args.chain}&address=${openseaSecond}`, SaleType.kOpenseaSecond // kOpenseaSecond
	...
	static async makeDAO(args: MakeDaoArgs, user?: string) {
		...
		if (user) {
			let [task] = await db.query<TaskData<MakeDAO>>(`select * from tasks where name like 'MakeDAO%' and state = 0 and user = ${escape(user)} limit 1`);
	...
	static async getTaskData(id: number) {
